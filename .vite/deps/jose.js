import { n as __export } from "./chunk-D94lAOSK.js";
...
var base64url_exports = /* @__PURE__ */ __export({
	decode: () => decode,
	encode: () => encode
});
...
async function cbcDecrypt(enc, cek, ciphertext, iv, tag$1, aad) {
...
async function gcmDecrypt(enc, cek, ciphertext, iv, tag$1, aad) {
	let encKey;
...
async function decrypt(enc, cek, ciphertext, iv, tag$1, aad) {
	if (!isCryptoKey(cek) && !(cek instanceof Uint8Array)) throw new TypeError(invalidKeyInput(cek, "CryptoKey", "KeyObject", "Uint8Array", "JSON Web Key"));
	...
	switch (enc) {
		...
		case "A256CBC-HS512":
			if (cek instanceof Uint8Array) checkCekLength(cek, parseInt(enc.slice(-3), 10));
			return cbcDecrypt(enc, cek, ciphertext, iv, tag$1, aad);
		case "A128GCM":
		case "A192GCM":
		case "A256GCM":
			if (cek instanceof Uint8Array) checkCekLength(cek, parseInt(enc.slice(1, 4), 10));
			return gcmDecrypt(enc, cek, ciphertext, iv, tag$1, aad);
		default: throw new JOSENotSupported(unsupportedEnc);
	}
}
...
async function decrypt$1(alg, key, encryptedKey) {
...
async function unwrap(alg, key, encryptedKey, iv, tag$1) {
	return decrypt(alg.slice(0, 7), key, encryptedKey, iv, tag$1, new Uint8Array());
}
...
async function decryptKeyManagement(alg, key, encryptedKey, joseHeader, options) {
	switch (alg) {
		...
		case "A256GCMKW": {
			...
			let tag$1;
			tag$1 = decodeBase64url(joseHeader.tag, "tag", JWEInvalid);
			return unwrap(alg, key, encryptedKey, iv, tag$1);
		}
...
async function flattenedDecrypt(jwe, key, options) {
	...
	if (jwe.tag !== void 0) tag$1 = decodeBase64url(jwe.tag, "tag", JWEInvalid);
	...
	const plaintext = await decrypt(enc, cek, ciphertext, iv, tag$1, additionalData);
...
var FlattenedEncrypt = class {
	...
	async encrypt(key, options) {
		...
		const { ciphertext, tag: tag$1, iv } = await encrypt(enc, plaintext, cek, this.#iv, additionalData);
...
var GeneralEncrypt = class {
	...
	async encrypt() {
		...
		if (this.#recipients.length === 1) {
			...
			if (flattened.aad) jwe$1.aad = flattened.aad;
			if (flattened.protected) jwe$1.protected = flattened.protected;
			if (flattened.unprotected) jwe$1.unprotected = flattened.unprotected;
			if (flattened.encrypted_key) jwe$1.recipients[0].encrypted_key = flattened.encrypted_key;
			if (flattened.header) jwe$1.recipients[0].header = flattened.header;
			return jwe$1;
		}
		let enc;
